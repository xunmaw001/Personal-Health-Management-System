	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.meishiLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('meishiLiuyan', this.meishiLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.meishiLiuyanPhoto ? this.detail.meishiLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('meishiLiuyanYuyue', {
									yonghuId: _this.user.id,
									meishiLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/meishiLiuyanYuyue/list');
							}
						}
					});
				},
