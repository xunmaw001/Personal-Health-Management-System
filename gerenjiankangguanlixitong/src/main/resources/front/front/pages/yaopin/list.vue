	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0 10rpx 0px rgba(0,0,0,0)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"40rpx","borderWidth":"2rpx 2rpx 10rpx 2rpx","fontSize":"26rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.yaopinName" type="text" placeholder="药品名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('yaopin','新增')" @click="onAddTap()"
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='yaopinTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in yaopinTypesList" :key="index" :class="yaopinTypes===item.codeIndex?'tabActive':''"
                                  @tap="yaopinTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <block v-for="(product,index) in list" :key="index">
        <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item"
            ...
            <image
                    ...
                    mode="aspectFill" :src="product.yaopinPhoto?baseUrl+product.yaopinPhoto.split(',')[0]:''">
            </image>
            <view class="list-item-body" style="display: flex;flex-wrap: wrap;">
                <view
                        ...
                        class="title">{{product.yaopinName}}
                </view>
                <view v-if="product.yaopinNewMoney"
                      ...
                      class="title">￥{{product.yaopinNewMoney}}
                </view>
                <view v-else-if="product.yaopinValue"
                      :style='{"padding":"12rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0","borderColor":"red","backgroundColor":"rgba(255, 255, 255, 1)","color":"rgba(0, 0, 0, 1)","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'
                      class="title">{{product.yaopinValue}}
                </view>
            </view>
            <view style="display: flex;justify-content: space-between;">
                <text v-if="isAuth('yaopin','修改')" class="cuIcon-edit"
                      @click.stop="onUpdateTap(product.id)">修改
                </text>

                <text v-if="isAuth('yaopin','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 药品类型 字段 字典表查询方法
            async yaopinTypesSelect(){
                let yaopinTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'yaopin_types',
                });
                yaopinTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.yaopinTypesList = yaopinTypes.data.list
            },
            //当前表的 药品类型字段类别搜索
			yaopinTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.yaopinTypes != 0) {
                    params.yaopinTypes = this.yaopinTypes
                }
                let _this = this
                let res = await this.$api.list(`yaopin`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yaopin', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.yaopinShenheTime=shijian;
                            await _this.$api.update(`yaopin`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.yaopinName) {
                    searchForm['yaopinName'] = this.searchForm.yaopinName
                }

                let res = await this.$api.list(`yaopin`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.yaopinContent!=null){
                        item.yaopinContent = item.yaopinContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
