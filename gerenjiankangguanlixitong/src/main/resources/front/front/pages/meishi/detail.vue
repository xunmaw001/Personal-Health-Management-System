			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
						...
						meishiLiuyanPingfenNumber: this.scoreData,
                        meishiLiuyanText: this.commentData,
                    }
                    await this.$api.save(`meishiLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('meishiLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						meishiId: this.meishiId
					});
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('meishi', _this.detail);
									_this.$utils.msg('取消成功');
