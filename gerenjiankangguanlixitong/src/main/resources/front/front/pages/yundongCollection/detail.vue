	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.yundongCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('yundongCollection', this.yundongCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.yundongCollectionPhoto ? this.detail.yundongCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('yundongCollectionYuyue', {
									yonghuId: _this.user.id,
									yundongCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/yundongCollectionYuyue/list');
							}
						}
					});
				},
