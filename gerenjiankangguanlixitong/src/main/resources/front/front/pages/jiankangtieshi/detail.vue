<template>
			<view style="margin: 0 30rpx;box-shadow:0 0px 0px 1rpx #a6a6a6">
				<view class="title">
					{{detail.jiankangtieshiName}}
				</view>
				<view style="font-weight: bold;font-size: 20upx;margin:0 30rpx 5rpx;"  :style="(detail.jiankangtieshiValue == null || detail.insertTime == null)?'text-align: center;':''">
					<text v-if="detail.jiankangtieshiValue">
						{{detail.jiankangtieshiValue}}
					</text>
					<text :style="detail.jiankangtieshiValue?'float: right':''">
						{{detail.insertTime}}<!-- .split(' ')[0] -->
					</text>
				</view>
				<view style="box-shadow: 0 0  0 1rpx #a6a6a6;margin: 0 30rpx"></view>
                <viwe>
					<image :src="baseUrl+detail.jiankangtieshiPhoto"></image>
                </viwe>
				<view class="content">
					<rich-text :nodes="detail.jiankangtieshiContent"></rich-text>
				</view>
			</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.jiankangtieshiId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('jiankangtieshi', this.jiankangtieshiId);
                    this.detail = res.data
					this.detail.jiankangtieshiContent = this.detail.jiankangtieshiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.jiankangtieshiPhoto ? this.detail.jiankangtieshiPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('jiankangtieshiYuyue', {
									yonghuId: _this.user.id,
									jiankangtieshiId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/jiankangtieshiYuyue/list');
							}
						}
					});
				},
