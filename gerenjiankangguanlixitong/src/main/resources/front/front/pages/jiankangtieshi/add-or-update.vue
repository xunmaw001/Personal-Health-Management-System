        ...
        <form class="app-update-pv">
            <view
                ...
                <input   disabled
                         ...
                         :disabled="ro.jiankangtieshiName" type="text" v-model="ruleForm.jiankangtieshiName" placeholder="贴士标题"></input>
            </view>
                <view :style='{"boxShadow":"0","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(228, 232, 230, 1)","margin":"0 0 0px 0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"100rpx"}'
                      class="cu-form-group">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
                          class="title">贴士类型</view>
                    <picker @change="jiankangtieshiTypesChange" :value="jiankangtieshiTypesIndex" :range="jiankangtieshiTypesOptions" range-key="indexName">
                        <view
                                :style='{"padding":"0 24rpx","boxShadow":"0 10rpx 0px rgba(0,0,0,0)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"40rpx","borderWidth":"2rpx 2rpx 10rpx 2rpx","fontSize":"26rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
                                class="uni-input">{{ruleForm.jiankangtieshiTypes?ruleForm.jiankangtieshiValue:"请选择贴士类型"}}</view>
                    </picker>
                </view>
                <view
                        ...
                        class="cu-form-group" @tap="jiankangtieshiPhotoTap">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
                          class="title">贴士图片</view>
                    <view class="right-input" style="padding:0;textAlign:left">
                        <image
                                ...
                                class="avator" v-if="ruleForm.jiankangtieshiPhoto" :src="baseUrl+ruleForm.jiankangtieshiPhoto" mode="aspectFill">
                        </image>
            ...
            <view
                ...
                <textarea :style='{"padding":"0 24rpx","boxShadow":"0 10rpx 0px rgba(0,0,0,0)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"40rpx","borderWidth":"2rpx 2rpx 10rpx 2rpx","fontSize":"26rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx","height":"200rpx"}'
                          :disabled="ro.jiankangtieshiContent" v-model="ruleForm.jiankangtieshiContent" placeholder="贴士详情"/>
            </view>
...
<script>
    ...
    export default {
		...
		/*下拉框*/
			let jiankangtieshiTypesParams = {
                page: 1,
                limit: 100,
                dicCode: 'jiankangtieshi_types',
            }
			let jiankangtieshiTypes = await this.$api.page(`dictionary`, jiankangtieshiTypesParams);
			this.jiankangtieshiTypesOptions = jiankangtieshiTypes.data.list
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`jiankangtieshi`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.jiankangtieshiId){
                this.ruleForm.jiankangtieshiId = options.jiankangtieshiId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
				if ((!this.ruleForm.jiankangtieshiName)) {
                    this.$utils.msg(`贴士标题不能为空`);
                    return
                }
				if ((!this.ruleForm.jiankangtieshiPhoto)) {
                    this.$utils.msg(`贴士图片不能为空`);
                    return
                }
                if (this.ruleForm.id) {
                    await this.$api.update(`jiankangtieshi`, this.ruleForm);
                } else {
                    await this.$api.save(`jiankangtieshi`, this.ruleForm);
                }
