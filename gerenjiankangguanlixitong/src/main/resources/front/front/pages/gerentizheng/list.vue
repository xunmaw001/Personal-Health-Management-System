<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]" style="width: 100%;">
        <view class="action" style="width: 100%;text-align: center;">
            <view :style="{width:'40rpx',borderRadius:'40rpx 0 0 40rpx',height:'80rpx',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
            </view>
            <view @tap="search"  :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'50%',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                搜索
            </view>
            <view v-if="isAuth('gerentizheng','新增')" @click="onAddTap()"
                   :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'50%',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                新增
            </view>
	...
	<view style="display: flex;flex-wrap: wrap">
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                ...
                <!-- 当前表 -->
                 <view v-show="item.gerentizhengNum" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">当前身高</text>
                     <text style="width: 70%">
                         <text v-if="item.gerentizhengNum != null">{{item.gerentizhengNum}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.tizhongNum" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">当前体重</text>
                     <text style="width: 70%">
                         <text v-if="item.tizhongNum != null">{{item.tizhongNum}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.shentiValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">身体状况</text>
                     <text style="width: 70%">{{item.shentiValue}}</text>
                 </view>
                <view v-show="item.gerentizhengContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">备注</text>
                    <view style="width: 70%" v-html="item.gerentizhengContent"></view>
                </view>
                <view v-show="item.gerentizhengTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">下次体检时间</text>
                    <text style="width: 70%">
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                res.data.list.forEach(function(item,index){
                    if(item.gerentizhengContent!=null){
                        item.gerentizhengContent = item.gerentizhengContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
                    }
                })
