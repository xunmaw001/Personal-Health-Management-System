			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.yundongName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
					...
					<view class="yinyuenei uni-padding-wrap uni-common-mt">
						<view>
							<video id="myVideo" style="preload:auto;margin: 20rpx;border-radius: 20rpx;" muted
								   :src="baseUrl+detail.yundongVideo" controls></video>
						</view>
					</view>
				</view>
				<!-- 简介 -->
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.yundongContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.yundongId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getyundongCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
                        yundongId: this.yundongId,
                        yonghuId: this.user.yonghuId,
						yundongLiuyanPingfenNumber: this.scoreData,
                        yundongLiuyanText: this.commentData,
                    }
                    await this.$api.save(`yundongLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('yundong', this.yundongId);
                    this.detail = res.data
					this.detail.yundongContent = this.detail.yundongContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.yundongPhoto ? this.detail.yundongPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getyundongCollection() {
					let params = {
						page: 1,
						limit: 1,
						yundongId: this.yundongId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`yundongCollection`, params);
					if (res.data.list.length == 1) {
						let yundongCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('yundongCollection', JSON.stringify([yundongCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getyundongCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('yundongCollection', {
									yonghuId: _this.user.id,
									yundongId: _this.detail.id,
									yundongCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('yundongLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						yundongId: this.yundongId
					});
				...
				async getThumbsup() {
					...
					let res = await _this.$api.list(`yundongCollection`, params);
					if (res.data.list.length > 0) {
						if (res.data.list[0].yundongCollectionTypes == '2') {
							_this.thumbsupFlag = true;
						}
						if (res.data.list[0].yundongCollectionTypes == '3') {
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('yundong', _this.detail);
									_this.$utils.msg('取消成功');
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('yundongYuyue', {
									yonghuId: _this.user.id,
									yundongId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/yundongYuyue/list');
							}
						}
					});
				},
