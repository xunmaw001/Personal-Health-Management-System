	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0 10rpx 0px rgba(0,0,0,0)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"40rpx","borderWidth":"2rpx 2rpx 10rpx 2rpx","fontSize":"26rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.yundongName" type="text" placeholder="标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('yundong','新增')" @click="onAddTap()"
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='yundongTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in yundongTypesList" :key="index" :class="yundongTypes===item.codeIndex?'tabActive':''"
                                  @tap="yundongTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <block v-for="(product,index) in list" :key="index">
        <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item"
            ...
            <view style="display: flex;justify-content: space-between;">
                <text v-if="isAuth('yundong','修改')" class="cuIcon-edit"
                      @click.stop="onUpdateTap(product.id)">修改
                </text>

                <text v-if="isAuth('yundong','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 运动类型字段类别搜索
			yundongTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.yundongTypes != 0) {
                    params.yundongTypes = this.yundongTypes
                }
                let _this = this
                let res = await this.$api.list(`yundong`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yundong', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.yundongShenheTime=shijian;
                            await _this.$api.update(`yundong`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
                ...
                let res = await this.$api.list(`yundong`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.yundongContent!=null){
                        item.yundongContent = item.yundongContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
