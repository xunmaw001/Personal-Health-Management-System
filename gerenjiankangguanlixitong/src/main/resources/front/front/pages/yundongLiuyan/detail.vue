	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.yundongLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('yundongLiuyan', this.yundongLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.yundongLiuyanPhoto ? this.detail.yundongLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('yundongLiuyanYuyue', {
									yonghuId: _this.user.id,
									yundongLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/yundongLiuyanYuyue/list');
							}
						}
					});
				},
