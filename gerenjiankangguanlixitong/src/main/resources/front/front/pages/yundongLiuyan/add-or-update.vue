        ...
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.yundongLiuyanText' placeholder="留言内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`yundongLiuyan`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.yundongId){
                this.ruleForm.yundongId = options.yundongId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.yundongLiuyanText)) {
                this.$utils.msg(`留言内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`yundongLiuyan`, this.ruleForm);
                } else {
                    await this.$api.save(`yundongLiuyan`, this.ruleForm);
                }
