            ...
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '计划标题' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.meirijihuaName" placeholder="计划标题" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('meirijihua','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/gerenjiankangguanlixitong/upload/meirijihuaMuBan.xls"
                        >批量导入每日计划数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('meirijihua','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="meirijihuaUploadSuccess"
                                :on-error="meirijihuaUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('meirijihua','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入每日计划数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('meirijihua','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "meirijihua.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="饮食计划">
                        <template slot-scope="scope">
                            <span v-if="scope.row.yinshiText != null &&scope.row.yinshiText.length>10">
                                {{scope.row.yinshiText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.yinshiText}}
                            </span>
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="运动计划">
                        <template slot-scope="scope">
                            <span v-if="scope.row.yundongText != null &&scope.row.yundongText.length>10">
                                {{scope.row.yundongText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.yundongText}}
                            </span>
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="药品计划">
                        <template slot-scope="scope">
                            <span v-if="scope.row.yaopinText != null &&scope.row.yaopinText.length>10">
                                {{scope.row.yaopinText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.yaopinText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('meirijihua','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('meirijihua','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('meirijihua','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                ...
                yundongText : null,
                yaopinText : null,
                meirijihuaContent : null,
                meirijihuaTime : null,
                meirijihuaDelete : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'meirijihua',//当前表表名,
                        sumColum : 'meirijihua_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'meirijihua_name',//分组字符串字段
                        // types : 'meirijihua_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.meirijihuaName!= '' && this.searchForm.meirijihuaName!= undefined) {
                    params['meirijihuaName'] = '%' + this.searchForm.meirijihuaName + '%'
                }
                                                                
                params['meirijihuaDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            meirijihuaUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "meirijihua/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "meirijihua/update",
                        method: "post",
                        data: {
                            id:id,
//                            meirijihuaTypes:meirijihuaTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
